...
library(readr)
library(pheatmap)
# This script reads in the rds experiments for the four experiments Done by Effie in 12/2022 and 3/2023 with Rb-Tnseq and does basic deseq comparison trials. As of 12/2025 we have not been using this file and have instead been using limmaVoom_full_table_2025.R. 

#setwd("~/Google\ Drive/My\ Drive/Utah_Professorship/projects/Tnseq/compiled_trials_3_2024/data/in_planta_rbtnseq_p25c2_dc3000")
setwd("/Users/talia/Documents/GitHub/TnSeq_Pseudomonas_Genotype/figures/2025_corrected")
...
#all_exp2 <- read.csv("/Users/talia/Library/CloudStorage/GoogleDrive-tkarasov@gmail.com/My Drive/Utah_Professorship/projects/Tnseq/compiled_trials_8_2025/full_experiments/all_p25_dc_axenic_924_2025.csv", header= TRUE)

all_exp <- read.table("/Users/talia/Library/CloudStorage/GoogleDrive-tkarasov@gmail.com/My Drive/Utah_Professorship/projects/Tnseq/compiled_trials_8_2025/full_experiments/all_p25_dc_axenic_10_07_2025.csv", header = TRUE, sep=",")
...
write_rds(count_table_filter, "~/Documents/GitHub/TnSeq_Pseudomonas_Genotype/input_data/full_experiments/all_count_table_filter_10_23_2025.rds")
...
plants <- c(rep("ey15_2",16), rep("col_0",16))
samps <- sample( plants, size=32, replace = F)
sample_order_filter[sample_order_filter$plant == "ctrl",]$plant <- samps

#now with the filtered dataset, make deseq object
cts<- count_table_filter[rowSums(is.na(count_table_filter)) == 0, , drop = FALSE]
storage.mode(cts) <- "integer"
dds <- DESeqDataSetFromMatrix(countData = cts, 
                              colData = sample_order_filter, design = ~ 
                                experiment + plant*time_point + treatment*time_point)
dds$time_point <- relevel(dds$time_point, "t3")
...
cd <- as.data.frame(sample_order_filter)
cd$experiment  <- factor(cd$experiment)
cd$plant       <- factor(cd$plant)
cd$treatment   <- factor(cd$treatment)
cd$time_point  <- factor(cd$time_point)
cd <- droplevels(cd)
dds$time_point <- relevel(dds$time_point, "t3")
dds <- DESeqDataSetFromMatrix(
  countData = cts,
  colData   = cd,
  design    = ~ experiment + plant*time_point + treatment*time_point
)

# LRT: drop ALL time-related terms
dds_lrt <- DESeq(dds, test = "LRT",
                 reduced = ~ experiment + plant + treatment)

res_anyTime <- results(dds_lrt)  # p-value tests "any time effect?"
alpha <- 0.05
sig_anyTime <- sum(res_anyTime$padj < alpha, na.rm = TRUE)
sig_anyTime
# 'sig_anyTime' = number of genes with any timepoint effect (including interactions)


# sig for plant
dds_plant <- DESeq(dds, test = "LRT",
                   reduced = ~ experiment + treatment*time_point)

res_plant <- results(dds_plant)
sig_plant <- sum(res_plant$padj < 0.05, na.rm = TRUE)
sig_plant

# list of significant genes
genes_plant <- rownames(res_plant)[which(res_plant$padj < 0.05)]

# sig for treatment
dds_treatment <- DESeq(dds, test = "LRT",
                       reduced = ~ experiment + plant*time_point)

res_treatment <- results(dds_treatment)
sig_treatment <- sum(res_treatment$padj < 0.05, na.rm = TRUE)
sig_treatment

# list of significant genes
genes_treatment <- rownames(res_treatment)[which(res_treatment$padj < 0.05)]


# now plant vs treatment only
plant_only <- setdiff(genes_plant, genes_treatment)
treatment_only <- setdiff(genes_treatment, genes_plant)
both <- intersect(genes_plant, genes_treatment)

c(plant_only = length(plant_only),
  treatment_only = length(treatment_only),
  both = length(both))

#########
# Let's do some graphing of the effect of strain
# --- 0) Ensure factors are set (and ordered if needed) ---
cd <- as.data.frame(colData(dds))
cd$experiment  <- droplevels(factor(cd$experiment))
cd$plant       <- droplevels(factor(cd$plant))
cd$treatment   <- droplevels(factor(cd$treatment))
cd$time_point  <- droplevels(factor(cd$time_point))
# optional: set explicit time order, e.g. t0 < t1 < t3
# cd$time_point <- factor(cd$time_point, levels = c("t0","t1","t3"))
colData(dds) <- S4Vectors::DataFrame(cd)

# --- 1) LRT: is treatment×time interaction significant? ---
dds_txint <- DESeq(dds, test = "LRT",
                   reduced = ~ experiment + plant*time_point + treatment + time_point)
res_txint <- results(dds_txint)

# pick top 10 by adjusted p-value (skip NAs)
top10 <- rownames(res_txint)[order(res_txint$padj, na.last = NA)][1:min(10, sum(!is.na(res_txint$padj)))]

# --- 2) Collect plotting data for those genes ---
get_pc <- function(g) {
  df <- plotCounts(dds, gene = g, intgroup = c("treatment","time_point"), returnData = TRUE)
  df$gene <- g
  df
}
plot_df <- dplyr::bind_rows(lapply(top10, get_pc))

# ensure factors are ordered as above
plot_df$treatment  <- droplevels(factor(plot_df$treatment, levels = levels(cd$treatment)))
plot_df$time_point <- droplevels(factor(plot_df$time_point, levels = levels(cd$time_point)))

# --- 3) Plot: points per sample, mean line per treatment, log scale for counts ---
p <- ggplot(plot_df, aes(x = time_point, y = count, color = treatment, group = treatment)) +
  geom_point(alpha = 0.45, position = position_jitterdodge(jitter.width = 0.15, dodge.width = 0.35), size = 1.6) +
  stat_summary(fun = mean, geom = "line", position = position_dodge(width = 0.35), linewidth = 0.7) +
  stat_summary(fun = mean, geom = "point", position = position_dodge(width = 0.35), size = 2.2) +
  scale_y_continuous(trans = "log10") +
  facet_wrap(~ gene, scales = "free_y", ncol = 5) +
  labs(title = "Top genes with significant treatment × time interaction",
       y = "Normalized counts (log10 scale)", x = "Time point", color = "Treatment") +
...
ggsave("tx_time_top10_small_multiples.pdf", p, width = 11, height = 6.5, units = "in")

#####
#Let's do some graphing for the effect of plant

dds_plantTime <- DESeq(dds, test = "LRT",
                       reduced = ~ experiment + treatment*time_point + plant + time_point)
res_plantTime <- results(dds_plantTime)

#-----------------------------------------------------------
# 3. Select the top 10 genes with smallest adjusted p-value
#-----------------------------------------------------------
top10 <- rownames(res_plantTime)[order(res_plantTime$padj, na.last = NA)][1:min(10, sum(!is.na(res_plantTime$padj)))]

#-----------------------------------------------------------
# 4. Collect normalized counts for those genes
#-----------------------------------------------------------
get_pc <- function(g) {
  df <- plotCounts(dds, gene = g, intgroup = c("plant", "time_point"), returnData = TRUE)
  df$gene <- g
  df
}
plot_df <- bind_rows(lapply(top10, get_pc))

#-----------------------------------------------------------
# 5. Plot: one facet per gene, colored by plant
#-----------------------------------------------------------
p <- ggplot(plot_df, aes(x = time_point, y = count, color = plant, group = plant)) +
  ...
  scale_y_continuous(trans = "log10") +
  facet_wrap(~ gene, scales = "free_y", ncol = 5) +
  labs(title = "Top genes with significant plant × time interaction",
       y = "Normalized counts (log10 scale)",
       x = "Time point", color = "Plant") +
  theme_bw(base_size = 11) +
  theme(panel.grid.minor = element_blank(),
        strip.background = element_rect(fill = "grey95"),
        legend.position = "bottom")
...
#-----------------------------------------------------------
# 1. Subselect p25.C2 and ask for Plant x time interaction
#-----------------------------------------------------------
...
# dds_red <- DESeq(dds, test="LRT", reduced = ~ experiment + plant*time_point + treatment + time_point)
...
# #BJEIHDPM_00932;product=putative glycosyltransferase. This result had changed on 10.23.2025
...
# res <- results(dds, name="time_pointt3.treatmentp25c2")
# 
# # or to shrink log fold changes association with condition:
# res_bac <- lfcShrink(dds, coef="time_pointt3.treatmentp25c2", type="apeglm")
...
# count_table_p25 <- count_table_p25[rowSums(is.na(count_table_p25)) == 0, , drop = FALSE]
# sample_order_p25 <- sample_order_filter[keep,]
# dds_p25 <- DESeqDataSetFromMatrix(countData = count_table_p25, 
#                               colData = sample_order_p25, design = ~  
...
# 
# # 1) Time effect (t3 vs t0) in col_0
# res_col0 <- results(dds_p25, name = "time_point_t3_vs_t0")
# 
# # 2) Time effect (t3 vs t0) in ey15_2  = main + interaction
# res_ey15 <- results(dds_p25, contrast = list(c("time_point_t3_vs_t0", "plantey15_2.time_pointt3")))
# 
# # 3) Difference in time effects between ey15_2 and col_0 (interaction only)
# res_diff <- results(dds_p25, name = "plantey15_2.time_pointt3")
# 
...
# res_plant <- res_diff 
#   #results(dds_p25, contrast=list(contrast))
