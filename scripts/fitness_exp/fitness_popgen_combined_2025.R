...
v <- voom(dge, design, block = p25_ey_metadata$experiment, correlation = corfit$consensus)
...
###### THIS IS WHERE I GENERATE THE LOGFC DATA#########
...
fit_con <- merge(conservation2, log2fc_df, by=0, all=TRUE)

# now we can ask how the time in tree relates to the fitness effect
model1 <- lm(data=fit_con, log2FC ~ Time.in.tree + Genetic_Diversity + Num_gene_events)
...
model2 <- lm(data=fit_exp, log2FC ~ Time.in.tree + KB_Pto +Genetic_Diversity + Col.0_Pto + Num_gene_events )
summary(model2)
...
model_formula <- log2FC ~ KB_Pto + Col.0_Pto +  Time.in.tree + Genetic_Diversity + Num_gene_events
...
# Use all cv_preds (since mtry is constant)
